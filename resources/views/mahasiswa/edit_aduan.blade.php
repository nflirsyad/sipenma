    ...
    <div class=" grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <h6 class="card-title">Form Edit Aduan</h6>
                ...
                <form class="forms-sample" method="POST" action="{{ route('update_aduan', $aduan->id) }}">
                    @csrf
                    @method('POST')
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <select class="form-select" name="jenis_aduan" id="jenisAduan" required>
                                @foreach($jenisAduan as $jenis)
                                    <option value="{{ $jenis->nama }}" {{ $aduan->jenis_aduan == $jenis->nama ? 'selected' : '' }}>{{ $jenis->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="judul_aduan" id="judul_aduan" value="{{ $aduan->judul_aduan }}" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="exampleInputEmail2" class="col-sm-3 col-form-label">Deskripsi</label>
                        <div class="col-sm-9">
                            <textarea type="text" class="form-control"  name="deskripsi"  id="deskripsi" autocomplete="off"  required>{{ $aduan->deskripsi }}</textarea>
                        </div>
                    </div>
                    <div class="row mb-3">
